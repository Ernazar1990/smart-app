...
import type { Subject, StrategicCombination, SubjectId } from "../constants";
import type { UserProgress } from "../types";

type ElectiveId = SubjectId | "creative";
...
const SubjectSelectionView: React.FC<SubjectSelectionViewProps> = ({
  ...
  const initialElectives = (user.chosenElectives || []) as ElectiveId[];

  const [selectedElectives, setSelectedElectives] = useState<ElectiveId[]>(
    initialElectives
  );
  const [isCreative, setIsCreative] = useState<boolean>(
    initialElectives.length === 1 && initialElectives[0] === "creative"
  );

  const electiveSubjects = useMemo(
    () => SUBJECTS.filter((s: Subject) => !!s.isElective),
    []
  );
  ...
  const toggleSubject = (id: SubjectId) => {
    ...
    if (selectedElectives.includes(id)) {
      ...
      const onlyReal = selectedElectives.filter(
        (x) => x !== "creative"
      ) as SubjectId[];

      if (onlyReal.length < 2) {
        setSelectedElectives([...onlyReal, id]);
      } else {
        // 2 болса соңғысын ауыстырамыз
        setSelectedElectives([onlyReal[1], id]);
      }
    }
  };

  const selectCombination = (ids: SubjectId[]) => {
    setIsCreative(false);
  ...
  const isComboSelected = (combo: StrategicCombination) => {
    const a = (selectedElectives.filter((x) => x !== "creative") as SubjectId[])
      .slice()
      .sort()
      .join("|");
    const b = combo.subjects.slice().sort().join("|");
    return !isCreative && a === b;
  };

  const comboBadge = (combo: StrategicCombination) => {
    // constants-та icon/color жоқ -> қарапайым badge
    const letters = combo.subjects
      .map((s) => SUBJECTS.find((x) => x.id === s)?.title?.slice(0, 1) ?? "•")
      .join("");
    return (
      <div className="w-16 h-16 rounded-[24px] bg-emerald-600 text-white flex items-center justify-center text-2xl font-black shadow-lg">
        {letters}
      </div>
    );
  };

  const subjectBadge = (sub: Subject) => {
    return (
      <div className="w-14 h-14 rounded-2xl bg-indigo-600 text-white flex items-center justify-center text-2xl font-black shadow-md">
        {sub.title?.slice(0, 1) ?? "S"}
      </div>
    );
  };

  const canSave = isCreative || selectedElectives.filter((x) => x !== "creative").length === 2;
    ...
    <div className="space-y-8 animate-in slide-in-from-bottom duration-500 pb-32">
      <header className="text-center space-y-3 px-4">
        ...
        <p className="text-gray-500 text-sm max-w-sm mx-auto leading-relaxed">
          ҰБТ-да тапсыратын бағытыңды таңда. Әр комбинация белгілі бір
          ...
          <section className="space-y-5">
            <div className="flex items-center gap-3 px-4">
              ...
              <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] whitespace-nowrap">
                Танымал комбинациялар
              </h4>
              <div className="h-0.5 flex-1 bg-gray-100"></div>
            </div>

            <div className="grid grid-cols-1 gap-4 px-2">
              {STRATEGIC_COMBINATIONS.map((combo: StrategicCombination) => {
                const selected = isComboSelected(combo);
                return (
                  <button
                    key={combo.id}
                    ...
                    <div className="relative z-10 flex items-center gap-6">
                      ...
                      <div className="flex-1 space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          {selected && (
                            <i className="fas fa-check-circle text-emerald-500 animate-in zoom-in"></i>
                          )}
                        </div>
                        <p className="text-xs text-gray-500 font-medium leading-relaxed line-clamp-2">
                          ...
                          {combo.subjects
                            .map(
                              (id) => SUBJECTS.find((s) => s.id === id)?.title ?? id
                            )
                            .join(" + ")}
                        </p>
                      </div>
          ...
          <section className="space-y-5 pt-4">
            <div className="flex items-center justify-between px-6">
              <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">
                Жеке пәндерді таңдау
              </h4>
              <span
                className={`text-[9px] font-black px-3 py-1 rounded-full transition-colors ${
                  selectedElectives.filter((x) => x !== "creative").length === 2
            ...
            <div className="grid grid-cols-2 gap-3 px-2">
              {electiveSubjects.map((sub: Subject) => {
                const selected =
                  !isCreative && selectedElectives.includes(sub.id);
                return (
                  <button
                    key={sub.id}
                  ...
                  >
                    {subjectBadge(sub)}
                    <span className="text-[11px] font-black text-gray-800 text-center uppercase tracking-tighter leading-tight">
                      {sub.title}
                    </span>
                  </button>
                );
        ...
        <div className="bg-white p-10 rounded-[50px] border border-gray-100 shadow-sm text-center animate-in zoom-in mx-2 space-y-8">
          ...
          <div className="space-y-3">
            <h4 className="text-2xl font-black text-gray-900">
              Шығармашылық емтихан бағыты
            </h4>
            <p className="text-gray-500 text-sm leading-relaxed max-w-[280px] mx-auto">
              Бұл бағытта сіз ҰБТ-да тек <b>3 міндетті пәннен</b> (Мат.
              сауаттылық, Оқу сауаттылығы, Қазақстан тарихы) тест тапсырасыз.
            </p>
          </div>
          <div className="bg-indigo-50 p-6 rounded-[35px] border border-indigo-100 flex flex-col gap-3">
            ...
            <p className="text-xs text-indigo-900 font-bold leading-relaxed">
              Екі таңдау пәнінің орнына жоғары оқу орнында екі шығармашылық
              емтихан тапсырасыз.
            </p>
          </div>
        </div>
      ...
      <div className="fixed bottom-6 left-6 right-6 z-50">
        <button
          disabled={!canSave}
          onClick={() => {
            onUpdateSubjects(selectedElectives as unknown as string[]);
            onClose();
          }}
        ...
        >
          {!canSave ? "2 пән таңдаңыз" : "Таңдауды сақтау"}
          {canSave ? <i className="fas fa-arrow-right text-xs"></i> : null}
        </button>
      </div>
    </div>
  );
