    ...
    return (
      <div className="space-y-8 pb-32 animate-in fade-in duration-500">
        <header className="px-2">
          <h2 className="text-3xl font-black text-gray-900 dark:text-white font-outfit tracking-tight">Дайындық 📚</h2>
          <p className="text-gray-500 dark:text-slate-500 text-sm mt-1">Оқуды жалғастыру үшін пәнді таңдаңыз.</p>
        </header>

        <section className="space-y-4">
          <div className="flex justify-between items-end px-2">
            <h4 className="text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-[0.2em]">Менің курстарым</h4>
          </div>
          <div className="grid grid-cols-1 gap-3">
            {mySubjects.map((sub) => (
              ...
              >
                <div className={`${sub.color} w-16 h-16 rounded-[24px] flex items-center justify-center text-white text-2xl shadow-lg group-hover:scale-105 transition-transform`}>
                  <i className={`fas ${sub.icon}`}></i>
                </div>
                <div className="flex-1">
                  <h5 className="font-black text-gray-900 dark:text-slate-100 text-lg font-outfit">{sub.name}</h5>
                  <p className="text-[10px] text-gray-400 uppercase font-black">{modules.length} модуль</p>
                </div>
  ...
  return (
    <div className="space-y-6 pb-24 animate-in fade-in">
      <header className="space-y-2">
        <button onClick={() => onSelectSubject('')} className="flex items-center gap-2 text-gray-400 font-black text-[10px] uppercase tracking-widest mb-2">
          <i className="fas fa-arrow-left"></i> Пәндерге оралу
        </button>
        <h2 className="text-3xl font-black text-gray-900 dark:text-white font-outfit">Оқу жоспары</h2>
        <p className="text-sm text-gray-500">Модульді басып, сабақтар тізімін көріңіз</p>
      </header>

      <div className="space-y-4">
        {modules.map((module, idx) => {
          const isExpanded = expandedModule === module.id;
          const isDone = module.lessons.length > 0 && module.lessons.every(l => user.completedLessons.includes(l.id));
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-xs ${
                    isDone ? 'bg-emerald-500 text-white' : isExpanded ? 'bg-white/20' : 'bg-gray-100 dark:bg-slate-900 text-gray-400'
                  }`}>
                    {isDone ? <i className="fas fa-check"></i> : (idx + 1)}
                  </div>
                  <div>
                    <h4 className="font-black text-sm font-outfit leading-tight">{module.title}</h4>
                    <span className="text-[9px] font-black uppercase tracking-widest opacity-60">
                      {module.lessons.length} сабақ
                    </span>
                  </div>
                </div>
              ...
              {isExpanded && (
                <div className="bg-gray-50 dark:bg-slate-900/50 rounded-[30px] border border-gray-100 dark:border-slate-800 p-2 space-y-1 animate-in slide-in-from-top duration-300">
                  {module.lessons.length === 0 ? (
                    <p className="p-6 text-center text-xs font-black text-gray-400 uppercase">Сабақтар әлі қосылмаған</p>
                  ) : (
                    module.lessons.map((lesson, lIdx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <h5 className="font-bold text-gray-700 dark:text-slate-300 text-sm group-hover:text-emerald-500">{lesson.title}</h5>
